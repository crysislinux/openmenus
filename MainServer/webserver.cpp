...
#endif /* !CONFIG_FILE */
WebServer::WebServer(const QString &param, QObject *parent) :
    QObject(parent)
{
...
void WebServer::start()
{
    ...
    if(!dir.exists("OpenMenus/menus")){
        if(!dir.mkpath("OpenMenus/menus")){
            QMessageBox::critical(0, tr("ManageMenus"), tr("Dir create error"), QMessageBox::Ok);
            return;
        }
    }
    dir.cd("OpenMenus/menus");
    QByteArray dirPath = dir.absolutePath().toLatin1();
    char *_dirPath = dirPath.data();
    char *_argv[]={"mongoose", "-document_root", _dirPath, NULL};
    startWebServer(2, _argv);
}
...
int WebServer::startWebServer(int argc, char *argv[]) {
  init_server_name();
